...
| favorite | boolean | お気に入り（☆）。既定 `false`。7-3 の一覧・7-4 の単語フォーム・7-6 の学習画面から付け外しする |
| createdAt | number | 作成日時（ms） |
...
| calendarStartDate | number \| null | null | 学習カレンダー（7-11）の表示開始日時（ms）。設定されていれば、カレンダーの表示と 3 つの数字はこの日時が属する日の 0:00 以降の ReviewLog だけを対象にする（押した時刻より前でも、同じ日の記録は数える）。null なら全期間。設定画面の「カレンダーの表示をリセット」で現在時刻を入れ、「全期間に戻す」で null に戻す（7-8）。ReviewLog は消さない |
| schemaVersion | number | 2 | バックアップ JSON の互換確認用。レコードの形が変わったときだけ上げる（4-7） |
...
Dexie の `version(n).stores()` を増やしていく。既存レコードの変換が必要なら `upgrade()` に書く。バックアップ JSON の `schemaVersion` はレコードの形に対応する番号で、インデックスの宣言だけを変えたバージョンでは上げない。項目が無いときに既定値で補えば読める追加（Settings の `importDelimiter` などや `calendarStartDate`）でも上げない。読み込み時に既定値で補う（10-2）ので、古いファイルも新しいファイルも同じ番号のまま読める（現在は 2）。

| バージョン | 変更 | upgrade |
|---|---|---|
| 1 | 初版 | — |
| 2 | Word に `favorite` を追加し、`favorite` と `[favorite+due]` のインデックスを足す | 既存の全 Word に `favorite = false` を入れる |
| 3 | `favorite` と `[favorite+due]` のインデックス宣言を外す（4-6） | 不要。レコードは変えない |
| 4 | Settings に `calendarStartDate` を追加する（4-5）。インデックスは v3 と同じ | 既存の Settings レコードに `calendarStartDate = null` を入れる |
...
| お気に入り復習 | 「★ お気に入り」一覧の開始ボタン（7-3） | favorite = true の単語のうち state ≠ 0 かつ due ≤ 今日の終わり | 同条件で再構築 |
| お気に入り New のみ | 「★ お気に入り」一覧の開始ボタン（復習 0 件のとき） | favorite = true の単語のうち state = 0 | 同条件で再構築 |

対象範囲は `scope` で表す。`'all'` = 全フォルダ、`'favorites'` = お気に入り、それ以外はフォルダ id。モード選択・結果画面の「もう一度」は `#/study/select?scope=<scope>` を引き継ぐ。
...
**終了（画面の ✕）**: このセッションで 1 件以上評価していれば、3 択のシート（7-6）を出す。まだ 1 件も評価していなければシートを出さず、その場でセッションを捨ててモード選択画面（`#/study/select?scope=<scope>`）へ戻る。結果画面は出さない。DB は何も変えていないので、書き戻しもアプリバッジの更新も行わない。

| 選択肢 | 処理 |
|---|---|
| 結果を保存して終了 | `endedEarly = true` で結果画面へ。評価済みの分はすでに保存されている。未評価の単語は何もしない |
| 結果を破棄して終了 | 下の `discard()`。結果画面は出さず、モード選択画面（`#/study/select?scope=<scope>`）へ戻る |
| キャンセル | 学習に戻る |

**評価の破棄 `discard()`**（「結果を破棄して終了」）は、`undo` スタックを全件巻き戻す処理として実装する:

1. `undo` が空なら何もしない（ボタンは非活性）。
2. Dexie のトランザクションで、`undo` に出てくる各単語の FSRS 項目をその単語の一番古い `before`（＝セッション開始前の値）に戻し、`undo` のすべての `logId` の ReviewLog を削除する。**この書き戻しと削除は 1 トランザクションで行う。** 書き込みに失敗したらセッション状態は変えず「取り消しに失敗しました」を表示する。
3. セッション状態は末尾から `undo()` の手順 3〜5 を繰り返したものにする（キュー・`items`・`index`・`lastDue` がセッション開始時点に戻る）。
4. アプリバッジを更新し（8-7）、セッションを捨ててモード選択画面へ戻る。

単語の本文・所属フォルダ・`favorite` は破棄の対象外で、そのまま残る。
...
| スワイプ | 表面・裏面のどちらでも有効。下 = Again、左 = Hard、右 = Good、上 = Easy。指が 10px 動いた時点で横か縦かを決め、以後はその軸で判定する（10px 未満で離したらタップ）。確定する移動量（以下「確定量」）はカード幅の 25%（390px 幅の画面ではカード幅 342px に対しておよそ 85px）で、縦横どちらの軸でもこの値を使う。確定量以上で指を離したら確定、未満なら元の位置に戻す。カード幅は指を置いた時点で測り、測れないときは 85px を使う |
| スワイプ中の見た目 | カードが指に追従する（決めた軸の方向）。移動量 0 から確定量にかけて、評価の色（`--again` / `--hard` / `--good` / `--easy`）をカードの背景として文字の下に敷き、不透明度を 0 から最大 0.85 まで変化させる。英単語・日本語訳・メモの文字は薄くしない。進み具合（移動量 ÷ 確定量）が 0.4 を超えたら文字色を白に切り替え、色の上でも読めるようにする。評価名（Again / Hard / Good / Easy）はカードの上に重ねず、評価色の文字で 1 行分の領域に表示する。上・左・右のスワイプ中はカード直下の領域に出し、下（Again）のスワイプ中は下がるカードに隠れないようカード上側の領域に出す（そのとき直下の領域は空）。指を離したら元（上側は空、直下は表面なら案内文、裏面なら空）に戻す。横スワイプでは最大 8 度カードを傾ける。確定量に達したら色を最大にして、指を離すとカードがその方向に抜けて次のカードが出る。左上の小さな評価表記は置かない |
...
| 単語一覧 | `#/folders/:folderId` | 検索、フィルター、並べ替え、一覧、開始ボタン、取込 |
| お気に入り一覧 | `#/favorites` | 全フォルダの ★ 付き単語。単語一覧と同じ画面で、「＋」と取込を出さない |
| 単語フォーム | `#/folders/:folderId/words/new`、`#/words/:wordId` | 登録と編集 |
| モード選択 | `#/study/select?scope=all`、`?scope=favorites`、または `?scope=<folderId>` | 3 モードとシャッフル |
| フラッシュカード | `#/study/flashcard` | セッション状態がなければホームへ |
| タイプ入力 | `#/study/typing` | 同上 |
| 結果 | `#/study/result` | 同上 |
| 設定 | `#/settings` | |
| 学習カレンダー | `#/calendar` | 学習した日を塗った月カレンダー、3 つの数字、日ごとの内訳（7-11） |

共通: 画面上部の上部バーは「左の領域・タイトル・右の領域」の 3 列にし、左右の領域は同じ固定幅（96px）にする。片側にしかアイコンが無い画面でも同じ幅を確保するので、タイトルは常に画面の中央に来る。左の領域に戻るボタン（ホーム以外）、右の領域に各画面のボタンを置く。ホームだけはタイトルを出さず（タイトルの列は空のまま残す）、左の領域に ⓘ とカレンダーのアイコン、右の領域に「＋」と歯車（設定）を置く（7-2）。左右の領域の幅（96px）はホームでも同じ。最大幅 480px で中央寄せ。

### 7-2. ホーム

| 要素 | 仕様 |
|---|---|
| 上部バー | タイトルは出さない（アプリ名も出さない）。左の領域に「ⓘ → カレンダー」、右の領域に「＋ → 歯車」の順で 2 つずつ並べる。ⓘ とカレンダーは 22px、「＋」は 22px、歯車は 26px。4 つともタップ領域は 44px。左右の領域は他の画面と同じ 96px（7-1） |
| 検索欄 | 画面上部（ヘッダーの下）。1 文字以上入力すると（入力から 300ms 後）、フォルダ一覧の代わりに全フォルダの単語から英語と日本語訳の部分一致（大文字小文字を無視）で検索した結果を表示する。各行: 一番左に ★（7-3 と同じ。★ 付きだけ表示し、それ以外も同じ幅を空けて文字の位置をそろえる）、英単語、日本語訳、フォルダ名。タップで単語フォーム（編集）へ。行を右にスワイプするとお気に入りを切り替える（7-3。削除の左スワイプはここには無い）。該当なしなら「該当する単語がありません」。結果が 100 件を超えるときは英単語順の先頭 100 件だけを表示し、絞り込みを促す。空にすると元の表示に戻る。サマリーカード、ストリーク、状態別割合は検索中もそのまま表示する |
...
| 説明（ⓘ） | 画面の左上端に置く ⓘ のボタン。押すと下から「状態と評価の説明」のシート（7-9 の InfoSheet）を出し、閉じるボタンで閉じる。ⓘ のアイコンは歯車より一回り小さい 22px、歯車は 26px にし、タップ領域はどちらも 44px のままにする |
| カレンダー | ⓘ の右に置くカレンダーのアイコン。ⓘ と同じ 22px（強調色の線で描いた図形）で、タップ領域は 44px。押すと学習カレンダー（`#/calendar`、7-11）へ |
| お気に入り | フォルダ一覧の一番上に常に置く、実体のないフォルダのカード。見た目は下のフォルダカードと同じで、アイコンは ★（Hard 色）、名前は「お気に入り」。件数は ★ 付きの単語数と、そのうち今日の復習数。「…」メニューは出さない。タップで `#/favorites`（7-3 のお気に入り一覧）へ |
| フォルダ一覧 | 各フォルダをフォルダのアイコン（📁）を付けた角丸のカード（枠線と薄い影あり）で表示し、単語一覧の行（区切り線のリスト行。7-3）と同じ見た目にならないようにする。カード: フォルダ名、総単語数、今日の復習数。タップで単語一覧へ。カード右端の「…」でメニューを出す。「…」はカードの上下中央にそろえ、タップ領域は 44px 以上にする（文字は行の下端に寄るので、点を 3 つ描いて中央にそろえる）。メニューは他の確認ダイアログと同じ枠・背景・角丸のシート（`<dialog>`）で、一番上にフォルダ名を見出しとして表示し、その下に区切り線を 1 本引く。続けて「名前を変更」（名前変更ダイアログ）、「削除」（確認ダイアログ。配下の単語と履歴も削除）、「キャンセル」を、それぞれ独立した枠線付き・角丸・高さ 48px・横幅いっぱいのボタンとして縦に並べ、ボタンの間に 8px の余白を取る。「名前を変更」は強調色（`--accent`）の枠と文字、「削除」は Again 色の枠と文字、「キャンセル」は薄い枠（`--border`）と通常の文字。いずれも背景は塗らず、押した瞬間に背景色が変わるようにする。行の間の区切り線は使わない |
| フォルダ追加 | 右上の「＋」。名前入力ダイアログ。バリデーションは 4-2。右上は「＋」と歯車を隣り合わせに置く |
| 学習を終える | 「今日の学習を終える」ボタン。通知が ON のときだけ表示。押すと第9章の処理。下に「最終予約: M/D HH:MM、N日分」または「まだ予約していません」 |
| 空状態 | フォルダ 0 件: 「★ お気に入り」のカードの下に「フォルダがありません。＋で追加してください」 |
...
| 並べ替え（ボタン） | フィルターの右に置く 1 つのボタン。左に基準の短い表示名、右端に矢印（↑ = 昇順、↓ = 降順）を出し（表示例: 登録日↓）、全体を 1 つの枠で囲む。内部は 2 つのタップ領域に分け、矢印の部分を押すと向きだけが切り替わり、基準名の部分を押すと基準を選ぶシートが下から出る。矢印のタップ領域は 44px 以上を確保する。ボタンの幅は固定し、既定の「登録日↓」を表示したときの幅を全基準で使う。これでフィルターの「すべて」「要復習」「習得済み」の幅は基準を変えても変わらない。文字サイズは基準によって変えない |
| 並べ替え（基準） | 基準は「登録日」「アルファベット」「学習状態」「次回の復習日」「お気に入り」の 5 つ。ボタンには短い表示名（登録日 = 登録日、アルファベット = A→Z、学習状態 = 状態、次回の復習日 = 復習日、お気に入り = ★）を出し、シートの中は上記の長い名前のままにする。シートの各行は高さ 48px 以上、行の間に区切り線、押した瞬間に背景色が変わる。現在の選択にチェックを付け、一番下に「キャンセル」を置く |
| 並べ替え（向き） | ボタン右端の矢印をワンタップで昇順と降順を切り替える。向きを反転すると、その基準の並びがそのまま逆順になる（同点の並び順や未学習の位置も反転する） |
| 各基準の並びと既定の向き | 登録日 = 新しい順（↓）。アルファベット = A→Z（↑）。学習状態 = New → Learning → Relearning → Review（↑。同じ状態内は due 昇順）。次回の復習日 = 近い順、未学習は最後（↑）。お気に入り = ★ 付きが上（↓。同じグループ内は登録日の新しい順）。基準を変えると、向きはその基準の既定に戻る |
| 並べ替えの既定と保存 | 既定は「登録日」の新しい順。基準も向きも保存せず、一覧を開くたびに既定に戻す。並べ替えはフィルターと検索の結果に対して適用する |
| 件数表示 | フィルターか検索が効いているとき「N語中M語を表示中」 |
| 状態別割合 | このフォルダの単語で 7-2 と同じ横棒 |
| 一覧の行 | 区切り線で区切ったリスト行（背景付きの角丸カードにはしない。ホームのフォルダカードと見分けるため）。行の一番左に ★（14px、Hard 色）を置き、★ 付きの単語だけ表示する。★ でない行も同じ幅を空けて、文字の位置がそろうようにする。続けて英単語、日本語訳、状態アイコン、復習予定。復習予定: state = 0 は「未学習」、due の日付が今日より前は「N日超過」、今日は「今日」、明日は「明日」、それ以外は「N日後」。行の並び順は「並べ替え」で選んだものに従う。タップで編集へ |
| 追加（＋） | 右上の「＋」を押すと、メニューを出さずに直接「単語を追加」の画面（単語フォーム。7-4）を開く。CSV/TSV 取込の入口は単語フォーム（新規）の中に置き、「＋」のメニューと「…」メニューは置かない |
| 選択（複数選択と一括移動・削除） | 右上の「選択」を押すと選択モードになり、各行の左にチェックボックスが出て、右上は「完了」に変わる。下部の開始ボタンは横に並べた 3 つのボタン「☆」「移動」「N件を削除」（N は選択数。いずれも 0 件なら非活性）に変わる。幅は「☆」を「移動」の半分にし、残りの幅を「移動」と「N件を削除」で等分する（3 つの比は 1 : 2 : 2）。高さは 3 つとも同じ。「☆」には文字を入れず ☆ か ★ だけを表示し、選択中の単語がすべて ★ なら ★ を出して「☆ を外す」、それ以外なら ☆ を出して「☆ を付ける」として動作し、実行後に画面下のトースト（移動と同じ位置・4 秒）で「N件をお気に入りに追加しました」または「N件をお気に入りから外しました」を出す。実行しても選択モードと選択は解除しない。「移動」を押すと移動先フォルダの一覧をダイアログ（見出し「N件の移動先」）で出し、選ぶと選択中の単語をまとめて移動する。一覧と「移動先が無い」の判定の両方から現在のフォルダを除き、現在のフォルダ以外が 0 件なら一覧の代わりに「移動先のフォルダがありません」を出す。ダイアログは他の確認ダイアログと同じ見た目（`<dialog>`、枠線なし）で、各行は枠線のないボタン。フォーカス枠はキーボード操作（`:focus-visible`）のときだけ出し、タップや、開いたときの自動フォーカスでは出さない（開いたときのフォーカスは最初の行ではなくダイアログ自身に置く。他のダイアログも同じ）。移動は folderId と updatedAt の更新のみで、FSRS の状態・ReviewLog・`favorite` は変えず、1 トランザクションで行う。移動後は選択モードを抜け、画面下（スワイプ削除の「元に戻す」と同じ位置のトースト）に「N件を「移動先のフォルダ名」に移動しました」を 4 秒表示する。「N件を削除」を押すと確認ダイアログ「N件の単語と学習履歴を削除します。よろしいですか？」の後に単語と ReviewLog を 1 トランザクションで削除する。★ 付きの単語も保護せず通常どおり削除するが、選択に ★ 付きが含まれるときは確認ダイアログの文面に「お気に入り N件を含みます」を添える（「N件の単語と学習履歴を削除します。お気に入り M件を含みます。よろしいですか？」）。「完了」で選択を解除してモードを抜ける。選択モード中はスワイプ削除を使わない。単語 0 件なら「選択」は非活性 |
| 左スワイプで削除 | ★ 付きでも扱いは変えない。行を左にスワイプすると、右端からゴミ箱アイコンが移動量に応じて徐々に現れる。行幅の 20% 以上スワイプした状態で指を離すと削除、20% 未満なら元の位置に戻す。確認ダイアログは出さず、行を一覧から隠して画面下に「削除しました　元に戻す」を 5 秒表示し、「元に戻す」を押すと行を戻す。実際の削除（単語と ReviewLog）はこの表示が消えた時点、または画面遷移（単語一覧のアンマウント）の時点で行う。横方向の移動が縦方向より大きい場合だけスワイプとして扱い、縦方向はブラウザのスクロールに任せる（`touch-action: pan-y`）。保留中に別の行をスワイプしたら、先の保留分を先に削除する |
| 右スワイプでお気に入り | 行を右にスワイプすると、左端から ☆ が移動量に応じて徐々に現れる。行幅の 20% 以上スワイプした状態で指を離すと favorite を切り替え（☆ なら付ける、★ なら外す）、20% 未満なら元の位置に戻す。★ 付きの行では現れるアイコンを ★ にして、外す動作だと分かるようにする。確認ダイアログも「元に戻す」も出さず、切り替え後に行の左の小さな ★ が即座に変わる。判定方式は左スワイプの削除と同じで、横方向の移動が縦方向より大きい場合だけスワイプとして扱う。お気に入り一覧（`#/favorites`）では ★ を外した行がその場で一覧から消える。ホームの横断検索の結果（7-2）にも同じ右スワイプを付ける（そちらに左スワイプの削除は無い） |
| 開始ボタン | 画面下部に固定。復習あり: 「学習開始（N語）」。復習 0 で New あり: 「新しい単語を学習（N語）」。どちらも 0: 非活性 |
| お気に入り一覧（`#/favorites`） | ホームの「★ お気に入り」カード（7-2）から開く、単語一覧と同じ画面。全フォルダの ★ 付き単語を出す。タイトルは「★ お気に入り」、戻るはホーム。各行にはフォルダ名を小さく添える。右上に「＋」は出さず、取込の入口も出さない（取込は単語フォーム（新規）の中だけ）。検索、フィルター、並べ替え、選択モード（☆ / 移動 / 削除）、左スワイプ削除、右スワイプのお気に入りは単語一覧と同じ。移動先の候補は全フォルダ（除くフォルダは無い）。下部の開始ボタンは、★ 付きの単語のうち今日の復習対象があれば「学習開始（N語）」、復習 0 で New があれば「新しい単語を学習（N語）」、どちらも 0 なら非活性。開始すると `#/study/select?scope=favorites`（6-1 のお気に入り復習 / お気に入り New のみ）へ。空状態は「お気に入りの単語がありません」 |
...
| お気に入り（☆）（編集のみ） | タイトルの右、上部バー（7-1）の右の領域の先頭（「保存」の左）に置く ☆ のボタン。学習画面（7-6）と同じ共通の部品を使う。直径 24px の円で囲み、中の星は約 16px にする。☆ のときは薄いグレー（ライト #E5E5EA、ダーク #3A3A3C）で塗った円の中に通常色の ☆、★ のときは Hard 色で塗った円の中に白の ★ を出す。枠線は付けない。タップ領域は 44px で、円はその中央に置き、上部バーの他のアイコンと縦の中心をそろえる。押すたびに ☆ と ★ が切り替わり、その場で保存する（保存ボタンを押さなくても反映され、キャンセル時の破棄確認の対象にもしない）。新規作成では表示しない |
...
| お気に入り（☆） | 上部バー（7-1）の右の領域に「☆ → ✕」の順で置く（左の領域は ↶）。単語フォーム（7-4）と同じ共通の部品を使う。直径 24px の円で囲み、中の星は約 16px にする。☆ のときは薄いグレー（ライト #E5E5EA、ダーク #3A3A3C）で塗った円の中に通常色の ☆、★ のときは Hard 色で塗った円の中に白の ★ を出す。枠線は付けない。タップ領域は 44px で、円はその中央に置き、上部バーの他のアイコンと縦の中心をそろえる。✕ や ↶ など他のアイコンは円で囲まない。押すたびに ☆ と ★ が切り替わり、その場で保存する。フラッシュカードは表面・裏面のどちらでも押せて、カードの反転・評価・キューには関わらない |
| 終了 | 右上の ✕ → 下から出す 3 択のシート（見出し「セッションを終了しますか？」）。このセッションの評価が 0 件のときはシートを出さず、その場でモード選択画面へ戻る（6-3）。フォルダのメニュー（7-2）と同じ枠線付き・角丸・高さ 48px・横幅いっぱいのボタンを 8px 空けて縦に並べる。上から「結果を保存して終了」（強調色 `--accent` の枠と文字）、「結果を破棄して終了」（Again 色の枠と文字。このセッションの評価が 0 件のときは非活性。0 件ではシート自体を出さないので、通常この状態にはならない）、「キャンセル」（薄い枠 `--border` と通常の文字）。それぞれの動作は 6-3 の「終了（画面の ✕）」 |
...
| 評価した単語 | 「次回最も早い復習日」の枠の下に置く枠。見出しは「評価した単語」。このセッションで評価した単語を出題順（キューに最初に現れた順）に一覧表示する。各行は左から、☆ ボタン（タップで favorite を切り替えてその場で保存する。☆ は薄いグレー `--muted`、★ は Hard 色。タップ領域は 44px）、英単語、日本語訳、最初に押した評価（Again / Hard / Good / Easy を 5-3 の評価色の文字で。再出題分の評価は使わない）、次回の復習（評価後の due から「今日」「明日」「N日後」。同じ単語を再出題でも評価したときは最後の評価の due）。英単語と日本語訳はそれぞれ 1 行に収め、長ければ末尾を省略する（`text-overflow: ellipsis`）。評価が 0 件のときはこの枠を出さない。「結果を破棄して終了」（6-3）した場合は結果画面自体を出さないので対象外 |
| ストリーク | 当日初の学習なら「🔥 N日連続学習！」 |
| 学習を終える | 通知が ON のとき「今日の学習を終える」ボタン（第9章）。ホームと同じ処理 |
| ボタンの配置 | 「ホームへ」と「もう一度」は縦並びにして画面の下部に固定し、それぞれ横幅いっぱい・高さ 48px 以上にする。下端はセーフエリア分の余白を取る。一番下に「ホームへ」（強調色 `--accent`）、その上に「もう一度」（枠線のみ）を置く。内訳や「評価した単語」の一覧が画面に収まらないときは画面全体がスクロールし、2 つのボタンはスクロールしても画面下部に固定されたままにする |
| ホームへ | ホームへ戻る |
...
| データ | 「バックアップを書き出す」「バックアップを読み込む」「進捗をリセット」「カレンダーの表示をリセット」の 4 つを、枠線・角丸・高さ 48px 以上・横幅いっぱいのボタンにする。書き出しと読み込みは強調色（`--accent`）の枠と文字、進捗リセットとカレンダーの表示リセットは Again 色の枠と文字で、いずれも背景は塗らず、押した瞬間に背景色が変わるようにする。CSV/TSV 取込の説明とバックアップの注意はボタンの下に置き、ボタンと区別できるよう薄い文字（`--muted`）のままにする。進捗リセットはフォルダ選択または「すべて」→ 確認 → 実行。「カレンダーの表示をリセット」は「進捗をリセット」の下に置き、押すと確認ダイアログ「カレンダーの表示をリセットしますか？これまでの学習記録は残りますが、カレンダーには今日以降の学習だけが表示されます。」を出し、実行すると `calendarStartDate`（4-5）に現在時刻を保存し、カレンダーにはリセットした日の 0:00 以降を表示する（朝に学習してから昼にリセットしても、その日の学習は表示に残る）。ReviewLog は削除しない。`calendarStartDate` が設定されているときは、ボタンの下に「M月D日以降を表示中」（`calendarStartDate` の日付）を薄い文字（`--muted`）で出し、その横に「全期間に戻す」のテキストボタンを置く。押すと確認なしで `calendarStartDate` を null に戻し、この行は消える |
...
| ProgressBar | 学習画面の進捗 |
| ConfirmDialog | 削除、リセット、変更の破棄の確認。`<dialog>` 要素を使う。外側タップはキャンセルと同じ |
| QuitSheet | 学習画面の ✕ の 3 択（7-6）。下から出すシート（`<dialog class="sheet">`）で、中身はフォルダのメニュー（7-2）と同じ枠線付きボタンの縦並び |
| EmptyState | 空状態の文言 |
...
外側タップ: アプリ内のダイアログとシート（上表の ConfirmDialog・QuitSheet・InfoSheet、フォルダ追加と名前変更の入力、フォルダの「…」メニュー、移動先の選択、並べ替えの基準選択）は、内容の外側（背景の暗い部分）をタップすると、何も変更せずにキャンセルと同じ動作で閉じる。タップの開始と終了の両方が外側だったときだけ閉じ、内容の上で押し始めて外側で離した場合は閉じない。取込画面（10-3）は全画面の作業なので対象外で、従来どおりキャンセルボタンでのみ戻る。また、下から出るシートのうち「並べ替えの基準選択」（7-3）と「InfoSheet（ⓘ の説明）」（上表）の 2 つは、見出し部分（「並べ替え」「状態と評価の説明」の行。選択肢や本文を含まない上部の領域）を引き下げても閉じる。見出しを下に引くとシートが指に追従して下に動き（上方向には動かさない）、80px 以上引いて離したとき、または短い距離でも速く下に払ったときは、シートを画面下まで滑らせてから、外側タップと同じキャンセルの動作で閉じる。80px 未満で離したら元の位置に戻す。引っ張っている間は背景の暗さを移動量に応じて薄くする。判定は Pointer Events と `setPointerCapture` で行い、引っ張れる領域には `touch-action: none` を付ける。InfoSheet の本文は今までどおりスクロールでき、本文を下に引いても閉じない。QuitSheet（学習画面の 3 択、7-6）は引き下げの対象にしない。
...
| --studied | #F2711C | #FF8A3D | 学習カレンダー（7-11）の学習日の塗り。Hard（`--hard`）と紛らわしくならないよう、少し赤寄りのオレンジにする |
| --bg | #FFFFFF | #000000 | 背景 |
| --fg | #1C1C1E | #F2F2F7 | 文字 |
...
学習した日（その日の ReviewLog がある日）を月カレンダーで振り返る画面。ホームの上部バーのカレンダーのアイコン（7-2）から開く。ルートは `#/calendar`。

| 要素 | 仕様 |
|---|---|
| 上部バー | タイトル「学習カレンダー」、左の領域に戻るボタン（ホームへ）。右の領域は空（7-1 の 3 列のまま） |
| 3 つの数字 | カレンダーの上に横 1 列で「今月の学習日数」「最長の連続日数」「総学習日数」を並べ、それぞれ「N日」と表示する。今月の学習日数 = 今日が属する月の学習日数（表示中の月を動かしても変わらない）。最長の連続日数 = 学習日が途切れず続いた最大の日数（全期間。月や年をまたいでも数える）。総学習日数 = 学習日の数。枠は結果画面の数字（7-7）と同じ見た目 |
| 月の見出し | 「YYYY年M月」。左に前月へ動かす「‹」、右に翌月へ動かす「›」（タップ領域 44px）。翌月は今月より先へは進めない（今月を表示中は「›」を非活性）。前月へはいくらでも戻れる。初期表示は今月。月を動かすと日付の選択は解除する |
| カレンダー | 日曜始まりの月カレンダー。最初の行に曜日（日〜土）を出し、月の外のマスは空にする。7 列は画面幅を等分し、マスの高さはタップ領域の 44px を確保する |
| 学習日の塗り | 学習した日のマスを学習日の色（`--studied`、7-10）で塗り、数字を白の太字にする。学習量による濃淡は付けず、1 件でも 100 件でも同じ見た目にする。学習していない日は塗らない |
| 今日 | 今日のマスを枠で囲む。塗っていない日は文字色（`--fg`）の 2px の枠、塗った日は学習日の色の縁の内側に背景色（`--bg`）の 2px の枠。どちらもマスの内側に描く |
| 選択中の日 | タップして選んだ日のマスを、マスの外側（マスの間の隙間）に描く強調色（`--accent`）の 2px の枠で囲む。塗りの上に重ねないので、塗った日でも塗っていない日でも見え、今日の枠（内側）とも重ならない |
| 日付のタップ | カレンダーの下にその日の学習記録をカードで表示する。もう一度同じ日を押すと選択を解除してカードを消す |
| 学習記録のカード | カレンダーとの間に 12px の余白を空けて置く。枠線（`--border`）・角丸・内側の余白（16px）を持ち、背景はカレンダー（`--bg`）と変えて面の色（ライト #F2F2F7、ダーク #1C1C1E）にする。括弧とスラッシュは使わない。1 行目: 見出しとして日付（「9月19日」）、右端に語数「N語」を大きめの太字で強調する。2 行目: Again / Hard / Good / Easy を等間隔の 4 列で横に並べ（並び順は結果画面 7-7 と同じ）、各列は上に件数を評価色（5-3）の大きめの数字、下にラベルを小さな薄い文字（`--muted`）で表示する。学習していない日は、カードの中に「M月D日」と「学習の記録はありません」（薄い文字）の 2 行だけを表示する |
| 内訳の数え方 | N = その日に評価した単語の数（同じ単語は 1 語）。4 列の件数 = 各単語のその日の最初の評価を Again / Hard / Good / Easy で数えたもの（再出題分は数えない。6-6 の評価内訳と同じ考え方）。4 列の合計は N になる |

集計（`src/app/calendar.ts`）:

1. ReviewLog を一度だけ読み込み、`review` をローカル日付キー（YYYY-MM-DD）に変換して日付ごとにまとめる。集計は DB を触らない純粋関数にする。
2. `calendarStartDate`（4-5）が設定されていれば、リセットした日の 0:00 以降（`review ≥ calendarStartDate の日の 0:00`）の ReviewLog だけを対象にする。同じ日なら押した時刻より前の記録も数え、前日以前の記録は数えない。null なら全期間。カレンダーの塗りと 3 つの数字の両方にこの絞り込みを適用する。
3. 進捗リセット（10-1）の有無は見ない。ReviewLog にある日はすべて学習日として扱い、リセット前の日も表示する。
4. ストリーク（6-7）とは別の集計で、互いに影響しない。`calendarStartDate` はホームのストリーク表示を変えない。
...
| 処理 | 対象単語の FSRS 項目を `newCardFields(now)` で初期化。ReviewLog と `favorite` は変えない。1 トランザクションで行う |
| 後処理 | バッジ更新 |
...
`<input type="file" accept="application/json,.json">` で選択。`app` と `schemaVersion` を確認し、`app` が違うか `schemaVersion` が 1 でも 2 でもなければ「このファイルは読み込めません」。`schemaVersion` 1 のファイルも読めるようにし、`favorite` が無いレコードは `false` として扱う（読み込んだ結果は現在のスキーマ 2 として書き込む）。settings は既定値（4-5）に重ねて読み、`calendarStartDate` が無い（または数値でない）ときは null（全期間）として扱う。確認ダイアログ「現在のデータをすべて置き換えます。よろしいですか？」の後、全テーブルを消して書き込む（置き換えのみ。マージはしない）。完了後にバッジ更新とホームへ遷移。
...
| 保存 | 「取り込む」を押すと、まず区切り・見出しの有無・列の割り当てを settings（4-5）に保存して次回の初期値にする。続けて `newCardFields()` と `favorite = false` を付けた単語を 1 トランザクションでまとめて追加し、フォルダ欄で選んだフォルダの単語一覧に戻って結果を表示する。バッジを更新する |
...
| `app/session.ts` | 初回評価のみ集計、Learning への遷移で末尾に再出題、完了判定、途中終了、正答率、取り消しで状態・履歴・再出題・集計が評価前に戻る、破棄で全評価が開始前に戻る、結果画面の一覧（評価した単語が出題順・最初の評価・評価後の due）、評価 0 件の終了は結果を生成しない |
| `app/streak.ts` | 空、連続、昨日まで、途切れ |
| `app/calendar.ts` | 日付ごとの集計（ローカル日付への変換、単語数と評価の内訳、同じ日の同じ単語は最初の評価だけ）、最長連続日数（空、途切れた区間の最長、月・年またぎ）、月またぎ（月の学習日数、月末と翌月 1 日の境目、前月・翌月の移動、日曜始まりの月カレンダー）、表示開始日の反映（null なら全期間、設定時はその日の 0:00 以降だけ、朝に学習して昼にリセットしてもその日を数える、3 つの数字にも反映）、学習記録の見出しの日付（M月D日） |
| `app/notify.ts` | 各日の件数（持ち越し含む）、0 件の日の除外、本文の書き分け、時刻の反映、URL エンコード、予約日数の上限 |
| `app/csv.ts` | カンマ / タブ判定、ヘッダー判定、引用符、重複、形式エラーの件数 |
| `app/backup.ts` | 書き出しと読み込みの往復、書き出す schemaVersion が 2、schemaVersion 1 のファイルの favorite を false として読む、calendarStartDate の往復と、無いファイルを null として読む、知らない schemaVersion の拒否 |
| `ui/screens/WordList.tsx` の `sortWords` | 5 つの基準それぞれの既定の並びと既定の向き、どの基準でも向きを反転すると並びがそのまま逆順になること（未学習の位置の反転を含む）、入力の配列を変えないこと。シートの長い名前とボタンの短い表示名。選択モードの文面（☆ の付け外し、★ を含む削除の確認） |
| `db/db.ts` | fake-indexeddb 上で、v2 の DB を最新（v4）で開くと `favorite` と `[favorite+due]` の宣言だけが減り、レコードはそのまま残ること。既存の Settings レコードに `calendarStartDate = null` が入ること |
| `db/repo.ts` | fake-indexeddb 上で、フォルダ削除のカスケード、復習対象クエリ、進捗リセット、評価の取り消し（FSRS 項目の書き戻しと ReviewLog の削除）、お気に入り（新規は false、☆ の付け外し、scope = 'favorites' のクエリ、フォルダ移動と進捗リセットで favorite が変わらないこと） |
...
| 4 | スワイプ評価 | 表面・裏面のどちらでも 4 方向が正しい評価になり、確定量（カード幅の 25%。およそ 85px）未満では元に戻る。表面のタップは裏返る。スワイプ中に画面がスクロールしたりラバーバンドしたりしない |
| 4-2 | 評価の取り消し | ↶ で直前の評価が取り消され、その単語が再び出る（フラッシュカードは裏面、タイプ入力は入力欄が空）。単語一覧の復習予定が評価前に戻る |
| 4-3 | 結果を破棄して終了 | ✕ の 3 択から「結果を破棄して終了」を選ぶと、結果画面を出さずにモード選択へ戻り、単語一覧の復習予定とアプリバッジがセッション開始前に戻る |
| 4-4 | 単語一覧の右スワイプ | 行を右にスワイプすると左端から ☆ が現れ、行幅の 20% 以上で離すと ★ が付く / 外れる。縦に指を動かしたときは一覧がそのままスクロールする |
...
export const scopeFolderId = (scope: Scope): string | null =>
  scope === 'all' || scope === FAVORITES ? null : scope;
...
export interface Word extends FsrsFields {
  ...
  memo: string;
  favorite: boolean;               // お気に入り（☆）。既定 false
  createdAt: number;
  updatedAt: number;
}
...
export type ImportDelimiter = ',' | '\t' | ';';          // CSV / TSV 取込の区切り文字
...
export interface Settings {
  ...
  importDelimiter: ImportDelimiter;       // 最後に使った区切り文字（10-3）
...
db.version(2)
  ...
  .upgrade((tx) =>
    tx.table('words').toCollection().modify((w: { favorite?: boolean }) => {
      w.favorite = false;
    }),
  );

// v3: favorite と [favorite+due] のインデックス宣言を外す（4-6、4-7）。
// レコードは変わらないので upgrade は要らない。
db.version(3).stores({
  folders: 'id, sortOrder',
  words: 'id, folderId, due, state, [folderId+due], [folderId+state]',
  reviewLogs: 'id, wordId, review',
...
// v4: Settings に calendarStartDate を足す（4-5、4-7）。既存レコードには null（全期間）を入れる。
db.version(4)
  .stores({
    folders: 'id, sortOrder',
    words: 'id, folderId, due, state, [folderId+due], [folderId+state]',
  ...
  .upgrade((tx) =>
    tx.table('settings').toCollection().modify((s: { calendarStartDate?: number | null }) => {
