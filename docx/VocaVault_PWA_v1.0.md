...
| schemaVersion | number | 2 | バックアップ JSON の互換確認用。Dexie のバージョンと同じ番号（4-7） |
...
| words | favorite | お気に入りの抽出（下記の注意） |
| words | [folderId+due] | フォルダ内の復習対象 |
| words | [folderId+state] | フォルダ内の New と状態別集計 |
| words | [favorite+due] | お気に入りの復習対象（下記の注意） |
| reviewLogs | wordId | 単語削除時のカスケード |
| reviewLogs | review | ストリーク計算 |

検索（英語・日本語訳の部分一致）はフォルダ内の単語をメモリに読み込んでフィルターする。数千語なら十分速い。

`favorite` と `[favorite+due]` は宣言するが、検索には使えない。IndexedDB のキーに真偽値を使えないためで、`where('favorite').equals(true)` は `DataError` になり、レコードもこのインデックスに載らない（レコードの追加自体は成功する）。お気に入りの抽出（7-2 のカード、7-3 の一覧、6-1 のセッション）は、検索と同じく読み込んだ単語を `favorite` でフィルターする。単語数が数千なら十分速い。
...
| 並べ替え（基準） | フィルターの右に置くボタン。押すと下から基準を選ぶシートを出す。基準は「登録日」「アルファベット」「学習状態」「次回の復習日」「お気に入り」の 5 つ。ボタンの表示は基準名のみ（例: 登録日）とし、向きは隣のボタンで示す。シートの各行は高さ 48px 以上、行の間に区切り線、押した瞬間に背景色が変わる。現在の選択にチェックを付け、一番下に「キャンセル」を置く |
| 並べ替え（向き） | 並べ替えボタンの隣に置く、↑（昇順）か ↓（降順）を表示するボタン。ワンタップで昇順と降順を切り替える。向きを反転すると、その基準の並びがそのまま逆順になる（同点の並び順や未学習の位置も反転する） |
...
| お気に入り（☆）（編集のみ） | タイトルの右に置く ☆ のボタン。押すたびに ☆（空）と ★（塗り、Hard 色）が切り替わり、その場で保存する（保存ボタンを押さなくても反映され、キャンセル時の破棄確認の対象にもしない）。新規作成では表示しない |
...
| お気に入り（☆） | 右上、✕ の左に置く ☆ のボタン。押すたびに ☆ と ★（Hard 色）が切り替わり、その場で保存する。フラッシュカードは表面・裏面のどちらでも押せて、カードの反転・評価・キューには関わらない |
...
interface Backup {
  app: 'VocaVault';
  schemaVersion: number;   // 書き出しは 2（Dexie のバージョンと同じ。4-7）
  exportedAt: number;
  folders: Folder[];
  words: Word[];
...
| `ui/screens/WordList.tsx` の `sortWords` | 5 つの基準それぞれの既定の並びと既定の向き、どの基準でも向きを反転すると並びがそのまま逆順になること（未学習の位置の反転を含む）、入力の配列を変えないこと。選択モードの文面（☆ の付け外し、★ を含む削除の確認） |
...
// 検索には使えない。お気に入りの抽出は読み込んだ配列を絞り込んで行う（4-6）。
db.version(2)
  .stores({
    folders: 'id, sortOrder',
