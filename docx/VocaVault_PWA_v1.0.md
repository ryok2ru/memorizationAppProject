...
| calendarStartDate | number \| null | null | 学習カレンダー（7-11）の表示開始日時（ms）。設定されていれば、カレンダーの表示と 3 つの数字はこの日時以降の ReviewLog だけを対象にする。null なら全期間。設定画面の「カレンダーの表示をリセット」で現在時刻を入れ、「全期間に戻す」で null に戻す（7-8）。ReviewLog は消さない |
...
| 学習日の塗り | 学習した日のマスを強調色（`--accent`）で塗り、数字を白の太字にする。学習量による濃淡は付けず、1 件でも 100 件でも同じ見た目にする。学習していない日は塗らない |
| 今日 | 今日のマスを枠で囲む。塗っていない日は文字色（`--fg`）の 2px の枠、塗った日は強調色の縁の内側に背景色（`--bg`）の 2px の枠 |
| 日付のタップ | カレンダーの下にその日の内訳を 1 行で表示する。学習した日は「M月D日: N語を学習（Again a / Hard b / Good c / Easy d）」、学習していない日は「M月D日: 学習の記録はありません」。もう一度同じ日を押すと選択を解除して内訳を消す |
| 内訳の数え方 | N = その日に評価した単語の数（同じ単語は 1 語）。a〜d = 各単語のその日の最初の評価を Again / Hard / Good / Easy で数えたもの（再出題分は数えない。6-6 の評価内訳と同じ考え方）。a + b + c + d = N になる |

集計（`src/app/calendar.ts`）:

1. ReviewLog を一度だけ読み込み、`review` をローカル日付キー（YYYY-MM-DD）に変換して日付ごとにまとめる。集計は DB を触らない純粋関数にする。
2. `calendarStartDate`（4-5）が設定されていれば、`review ≥ calendarStartDate` の ReviewLog だけを対象にする（同じ日でもその時刻より前の記録は数えない）。null なら全期間。カレンダーの塗りと 3 つの数字の両方にこの絞り込みを適用する。
...
| `app/calendar.ts` | 日付ごとの集計（ローカル日付への変換、単語数と評価の内訳、同じ日の同じ単語は最初の評価だけ）、最長連続日数（空、途切れた区間の最長、月・年またぎ）、月またぎ（月の学習日数、月末と翌月 1 日の境目、前月・翌月の移動、日曜始まりの月カレンダー）、表示開始日の反映（null なら全期間、設定時はその日時以降だけ、3 つの数字にも反映）、内訳の文面 |
