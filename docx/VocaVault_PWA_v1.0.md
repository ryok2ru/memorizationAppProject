...
| 終了 | 右上の ✕ → 下から出す 3 択のシート（見出し「セッションを終了しますか？」）。フォルダのメニュー（7-2）と同じ枠線付き・角丸・高さ 48px・横幅いっぱいのボタンを 8px 空けて縦に並べる。上から「結果を保存して終了」（強調色 `--accent` の枠と文字）、「結果を破棄して終了」（Again 色の枠と文字。このセッションの評価が 0 件のときは非活性）、「キャンセル」（薄い枠 `--border` と通常の文字）。それぞれの動作は 6-3 の「終了（画面の ✕）」 |
...
外側タップ: アプリ内のダイアログとシート（上表の ConfirmDialog・QuitSheet・InfoSheet、フォルダ追加と名前変更の入力、フォルダの「…」メニュー、移動先の選択、並べ替えの基準選択）は、内容の外側（背景の暗い部分）をタップすると、何も変更せずにキャンセルと同じ動作で閉じる。タップの開始と終了の両方が外側だったときだけ閉じ、内容の上で押し始めて外側で離した場合は閉じない。取込画面（10-3）は全画面の作業なので対象外で、従来どおりキャンセルボタンでのみ戻る。
...
| `app/session.ts` | 初回評価のみ集計、Learning への遷移で末尾に再出題、完了判定、途中終了、正答率、取り消しで状態・履歴・再出題・集計が評価前に戻る、破棄で全評価が開始前に戻る、結果画面の一覧（評価した単語が出題順・最初の評価・評価後の due） |
