...
| 操作 | 処理 |
|---|---|
| 表面 | カードに英単語を表示し、カードの直下に設けた高さ固定の 1 行分の領域に案内文「タップで答えを表示。スワイプで評価」を表示。タップで裏面に反転。裏面には英単語（小）、日本語訳（大）、メモ（灰色）を表示し、直下の領域は空にする |
...
| スワイプ中の見た目 | カードが指に追従する（決めた軸の方向）。移動量 0 から 120px にかけて、評価の色（`--again` / `--hard` / `--good` / `--easy`）をカードの背景として文字の下に敷き、不透明度を 0 から最大 0.85 まで変化させる。英単語・日本語訳・メモの文字は薄くしない。進み具合（移動量 ÷ 120px）が 0.4 を超えたら文字色を白に切り替え、色の上でも読めるようにする。評価名（Again / Hard / Good / Easy）はカードの上に重ねず、カード直下の 1 行分の領域に評価色の文字で表示し、指を離したら元（表面は案内文、裏面は空）に戻す。横スワイプでは最大 8 度カードを傾ける。120px に達したら色を最大にして、指を離すとカードがその方向に抜けて次のカードが出る。左上の小さな評価表記は置かない |
...
| 選択（複数選択と一括移動・削除） | 右上の「選択」を押すと選択モードになり、各行の左にチェックボックスが出て、右上は「完了」に変わる。下部の開始ボタンは横に並べた 2 つのボタン「移動」と「N件を削除」（N は選択数。どちらも 0 件なら非活性）に変わる。「移動」を押すと移動先フォルダの一覧（現在のフォルダを除く。他にフォルダが無ければ「移動先のフォルダがありません」）をダイアログで出し、選ぶと選択中の単語をまとめて移動する。移動は folderId と updatedAt の更新のみで、FSRS の状態と ReviewLog は変えず、1 トランザクションで行う。移動後は選択モードを抜け、「N件を移動しました」を 3 秒表示する。「N件を削除」を押すと確認ダイアログ「N件の単語と学習履歴を削除します。よろしいですか？」の後に単語と ReviewLog を 1 トランザクションで削除する。「完了」で選択を解除してモードを抜ける。選択モード中はスワイプ削除を使わない。単語 0 件なら「選択」は非活性 |
...
| 保存 | 必須項目が trim 後に空、または文字数超過なら非活性。超過項目の下に赤字で理由を表示。新規は `newCardFields()` を付けて保存。保存後はフォルダ欄で選んだフォルダの単語一覧へ戻る（フォルダを変えた場合は移動先の一覧） |
...
単語一覧の「＋」→「CSV/TSV を取り込む」でファイルを選ぶと、単語一覧の代わりに取込画面を表示する。「取り込む」を押すまで何も書き込まない。
...
| 保存 | 「取り込む」を押すと、まず区切り・見出しの有無・列の割り当てを settings（4-5）に保存して次回の初期値にする。続けて `newCardFields()` を付けた単語を 1 トランザクションでまとめて追加し、単語一覧に戻って結果を表示する。バッジを更新する |
| 結果 | 「N件を取り込みました（スキップ M件: 重複 a件、形式エラー b件）」 |
| キャンセル | 何も書き込まず単語一覧に戻る。settings も変えない |
