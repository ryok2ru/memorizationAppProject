...
| 操作 | 処理 |
|---|---|
| 表面 | 英単語と案内文「タップで答えを表示。スワイプで評価」を表示。タップで裏面に反転。裏面には英単語（小）、日本語訳（大）、メモ（灰色）を表示 |
...
| スワイプ中の見た目 | カードが指に追従する（決めた軸の方向）。移動量 0 から 120px にかけて、カード全体の背景が評価の色（`--again` / `--hard` / `--good` / `--easy`）に不透明度 0 から 0.9 まで変化し、カード中央に評価名（Again / Hard / Good / Easy）を大きく白文字で表示する（不透明度は背景と同じ進み方）。横スワイプでは最大 8 度カードを傾ける。120px に達したら色を最大にして、指を離すとカードがその方向に抜けて次のカードが出る。左上の小さな評価表記は置かない |
...
| 選択（複数選択と一括削除） | 右上の「選択」を押すと選択モードになり、各行の左にチェックボックスが出て、右上は「完了」に変わる。下部の開始ボタンは「N件を削除」（N は選択数。0 件なら非活性）に変わり、押すと確認ダイアログ「N件の単語と学習履歴を削除します。よろしいですか？」の後に単語と ReviewLog を 1 トランザクションで削除する。「完了」で選択を解除してモードを抜ける。選択モード中はスワイプ削除を使わない。単語 0 件なら「選択」は非活性 |
...
| メモ | 任意。複数行。1000 文字まで |
| 取込への導線（新規のみ） | メモ欄の下に薄い文字のリンク「複数の単語をまとめて登録する（CSV / TSV 取込）」を置く。押すとファイル選択の後に取込画面（10-3）へ遷移する。取込画面のキャンセルはこのフォームに戻る。編集（既存の単語）では表示しない |
