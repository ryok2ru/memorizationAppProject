...
| 要素 | 仕様 |
|---|---|
| 検索欄 | 画面上部（ヘッダーの下）。1 文字以上入力すると（入力から 300ms 後）、フォルダ一覧の代わりに全フォルダの単語から英語と日本語訳の部分一致（大文字小文字を無視）で検索した結果を表示する。各行: 一番左に ★（7-3 と同じ。★ 付きだけ表示し、それ以外も同じ幅を空けて文字の位置をそろえる）、英単語、日本語訳、フォルダ名。タップで単語フォーム（編集）へ。該当なしなら「該当する単語がありません」。結果が 100 件を超えるときは英単語順の先頭 100 件だけを表示し、絞り込みを促す。空にすると元の表示に戻る。サマリーカード、ストリーク、状態別割合は検索中もそのまま表示する |
...
| お気に入り一覧（`#/favorites`） | ホームの「★ お気に入り」カード（7-2）から開く、単語一覧と同じ画面。全フォルダの ★ 付き単語を出す。タイトルは「★ お気に入り」、戻るはホーム。各行にはフォルダ名を小さく添える。右上に「＋」は出さず、取込の入口も出さない（取込は単語フォーム（新規）の中だけ）。検索、フィルター、並べ替え、選択モード（☆ / 移動 / 削除）、左スワイプ削除は単語一覧と同じ。移動先の候補は全フォルダ（除くフォルダは無い）。下部の開始ボタンは、★ 付きの単語のうち今日の復習対象があれば「学習開始（N語）」、復習 0 で New があれば「新しい単語を学習（N語）」、どちらも 0 なら非活性。開始すると `#/study/select?scope=favorites`（6-1 のお気に入り復習 / お気に入り New のみ）へ。空状態は「お気に入りの単語がありません」 |
