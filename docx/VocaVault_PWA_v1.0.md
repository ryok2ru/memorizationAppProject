...
| 並べ替え（ボタン） | フィルターの右に置く 1 つのボタン。左端に矢印（↑ = 昇順、↓ = 降順）、その右に基準の短い表示名を出し、全体を 1 つの枠で囲む。内部は 2 つのタップ領域に分け、矢印の部分を押すと向きだけが切り替わり、基準名の部分を押すと基準を選ぶシートが下から出る。矢印のタップ領域は 44px 以上を確保する。ボタンの幅は固定し、既定の「↓登録日」を表示したときの幅を全基準で使う。これでフィルターの「すべて」「要復習」「習得済み」の幅は基準を変えても変わらない。文字サイズは基準によって変えない |
...
| 並べ替え（向き） | ボタン左端の矢印をワンタップで昇順と降順を切り替える。向きを反転すると、その基準の並びがそのまま逆順になる（同点の並び順や未学習の位置も反転する） |
...
| お気に入り（☆）（編集のみ） | タイトルの右、上部バー（7-1）の右の領域の先頭（「保存」の左）に置く ☆ のボタン。学習画面（7-6）と同じ共通の部品を使う。直径 36px の円で囲み、☆ のときは薄いグレー（ライト #E5E5EA、ダーク #3A3A3C）で塗った円の中に通常色の ☆、★ のときは Hard 色で塗った円の中に白の ★ を出す。枠線は付けない。タップ領域は 44px。押すたびに ☆ と ★ が切り替わり、その場で保存する（保存ボタンを押さなくても反映され、キャンセル時の破棄確認の対象にもしない）。新規作成では表示しない |
...
| お気に入り（☆） | 上部バー（7-1）の右の領域に「☆ → ✕」の順で置く（左の領域は ↶）。単語フォーム（7-4）と同じ共通の部品を使う。直径 36px の円で囲み、☆ のときは薄いグレー（ライト #E5E5EA、ダーク #3A3A3C）で塗った円の中に通常色の ☆、★ のときは Hard 色で塗った円の中に白の ★ を出す。枠線は付けない。タップ領域は 44px。✕ や ↶ など他のアイコンは円で囲まない。押すたびに ☆ と ★ が切り替わり、その場で保存する。フラッシュカードは表面・裏面のどちらでも押せて、カードの反転・評価・キューには関わらない |
...
| ProgressBar | 学習画面の進捗 |
| ConfirmDialog | 削除、リセット、変更の破棄の確認。`<dialog>` 要素を使う |
