...
| フォルダ一覧 | 各フォルダをフォルダのアイコン（📁）を付けた角丸のカード（枠線と薄い影あり）で表示し、単語一覧の行（区切り線のリスト行。7-3）と同じ見た目にならないようにする。カード: フォルダ名、総単語数、今日の復習数。タップで単語一覧へ。カード右端の「…」で編集（名前変更ダイアログ）と削除（確認ダイアログ。配下の単語と履歴も削除） |
...
| 一覧の行 | 区切り線で区切ったリスト行（背景付きの角丸カードにはしない。ホームのフォルダカードと見分けるため）。英単語、日本語訳、状態アイコン、復習予定。復習予定: state = 0 は「未学習」、due の日付が今日より前は「N日超過」、今日は「今日」、明日は「明日」、それ以外は「N日後」。タップで編集へ |
