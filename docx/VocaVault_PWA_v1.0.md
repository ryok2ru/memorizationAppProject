...
| 追加（＋） | 右上の「＋」を押すとメニュー「単語を追加」「CSV/TSV を取り込む」を出す（`<dialog>` のアクションシート）。「単語を追加」は単語フォームへ、「CSV/TSV を取り込む」はファイル選択の後に取込画面（10-3）へ。「…」メニューは置かない |
...
| 左スワイプで削除 | 行を左にスワイプすると、右端からゴミ箱アイコンが移動量に応じて徐々に現れる。行幅の 40% 以上スワイプした状態で指を離すと削除、40% 未満なら元の位置に戻す。確認ダイアログは出さず、行を一覧から隠して画面下に「削除しました　元に戻す」を 5 秒表示し、「元に戻す」を押すと行を戻す。実際の削除（単語と ReviewLog）はこの表示が消えた時点、または画面遷移（単語一覧のアンマウント）の時点で行う。横方向の移動が縦方向より大きい場合だけスワイプとして扱い、縦方向はブラウザのスクロールに任せる（`touch-action: pan-y`）。保留中に別の行をスワイプしたら、先の保留分を先に削除する |
