...
| importDelimiter | "," \| "\t" \| ";" | "," | CSV / TSV 取込で最後に使った区切り文字。取込画面の初期値に使う（10-3） |
| importHasHeader | boolean | false | 取込で最後に使った「1行目は見出し」 |
| importColumns | ("en" \| "ja" \| "memo" \| "skip")[] | ["en", "ja", "memo"] | 取込で最後に使った列の割り当て（左の列から順） |
...
| learning_steps | 10 分 2 段 | ts-fsrs 5.4 は Good で「次のステップ」に進む扱いのため、1 段だと New に Good で直接 Review に進んでしまう。2 段にすると、新しい単語は Good でセッション内で 1 回再出題されてから Review に進む。調査資料が推奨する学習直後の再想起に対応する |
...
|---|---|---|---|
| Again | 1 | Again | Again 色 |
| Hard | 2 | Hard | Hard 色 |
| Good | 3 | Good | Good 色 |
| Easy | 4 | Easy | Easy 色 |

UI 表示はフラッシュカードの評価ボタン、タイプ入力の Good / Easy ボタン、結果画面の内訳で同じ表記を使う。
...
状態遷移は ts-fsrs に任せる。概略（learning_steps は 10 分 2 段、relearning_steps は 10 分 1 段）: New → Good → Learning（10 分後、2 段目）→ Good/Easy → Review。New → Hard → Learning（10 分後、1 段目）→ Good → Learning（10 分後、2 段目）→ Good → Review。New → Easy → Review。Review → Again → Relearning（10 分後）→ Good/Easy → Review。Learning/Relearning → Again → 同じ状態で 1 段目からやり直し。Learning/Relearning → Hard → 同じ段にとどまる（Learning は 10 分後、Relearning は 15 分後）。
...
| `preview(word, now): Record<Rating, { due: number; requeue: boolean; label: string }>` | `scheduler.repeat()` の結果から4評価の次回日時、再出題の有無、ラベル（「↻」「3日後」）を返す。`requeue` は更新後の state が 1 か 3（6-3 手順 4 の再出題条件と同じ） |
| `retrievability(word, now): number` | `scheduler.get_retrievability(card, date, false)` |

プレビューのラベル: 更新後の state が 1 か 3（10 分ステップで当日中に再出題される）なら「↻」だけを表示する。それ以外は評価の結果が 1 日以上先なので、`scheduled_days` を使って「N日後」。分単位の表示（「10分後」など）や「今日」は使わない。
...
interface SessionState {
  ...
  finishedAt?: number;
  endedEarly: boolean;
  undo: UndoEntry[];                       // 取り消しスタック。末尾が直前の評価
}

interface UndoEntry {
  wordId: string;
  before: FsrsFields;                      // 評価前の FSRS 項目（4-3 の due 以降）
  logId: string;                           // その評価で追加した ReviewLog の id
  requeued: boolean;                       // その評価でキュー末尾に再出題を追加したか
  setFirstRating: boolean;                 // その評価で firstRating を設定したか
  index: number;                           // 評価前の index
}
```
...
6. 手順 1〜5 で変えた内容を `UndoEntry` として `undo` の末尾に積む。

再出題は due を待たずに順番が来たら出す（学習中カードを前倒しで出す扱い）。キューの最後の1枚が再出題になった場合は連続して同じ単語が出るが、許容する。

**評価の取り消し `undo()`**（学習画面の ↶ ボタン。7-6）:

1. `undo` が空なら何もしない（ボタンは非活性）。空でなければ末尾の `UndoEntry` を取り出す。
2. Dexie のトランザクションで、対象単語の FSRS 項目を `before` に戻し、`logId` の ReviewLog を削除する。**この 2 つは 1 トランザクションで行う。** 書き込みに失敗したらセッション状態は変えず「取り消しに失敗しました」を表示する。
3. `requeued` なら、その評価でキュー末尾に追加した `wordId` を取り除く。
4. `setFirstRating` なら `items[wordId].firstRating` を未設定に戻す（結果画面の集計から外れる）。
5. `index` を `UndoEntry.index`（評価前の位置）に戻す。`finishedAt` は消す。
6. 表示: フラッシュカードは対象単語の裏面（評価ボタンが有効な状態）に戻す。タイプ入力は対象単語の出題状態（入力欄が空）に戻し、答え直せるようにする。

セッション開始時点まで何回でも戻れる。取り消しの後に評価すれば通常どおり `undo` に積まれるので、やり直しを繰り返せる。ページを再読み込みしたら `undo` は消えてよい（評価済みの分は保存されたまま残る）。
...
| 操作 | 処理 |
|---|---|
| 表面 | カードに英単語を表示し、カードの直下に設けた高さ固定の 1 行分の領域に案内文「タップで答えを表示。スワイプで評価」を表示。タップで裏面に反転。裏面には英単語（小）、日本語訳（大）、メモ（灰色）を表示し、直下の領域は空にする。カードの上側にも同じ高さ固定の 1 行分の領域を設け、スワイプしていないときは表面・裏面とも空のままにする |
| 評価ボタン（4つ） | 裏面だけに表示する（表面では場所だけ確保して表示しない）。表示は Again / Hard / Good / Easy（5-3）。各ボタンに `preview()` のラベル（「↻」か「N日後」）を小さく表示。いずれかのボタンが「↻」のときは、ボタンの下に薄い文字で 1 行「↻ このセッションでもう一度出ます」を表示する |
| スワイプ | 表面・裏面のどちらでも有効。下 = Again、左 = Hard、右 = Good、上 = Easy。指が 10px 動いた時点で横か縦かを決め、以後はその軸で判定する（10px 未満で離したらタップ）。移動量 120px 以上で指を離したら確定、未満なら元の位置に戻す |
| スワイプ中の見た目 | カードが指に追従する（決めた軸の方向）。移動量 0 から 120px にかけて、評価の色（`--again` / `--hard` / `--good` / `--easy`）をカードの背景として文字の下に敷き、不透明度を 0 から最大 0.85 まで変化させる。英単語・日本語訳・メモの文字は薄くしない。進み具合（移動量 ÷ 120px）が 0.4 を超えたら文字色を白に切り替え、色の上でも読めるようにする。評価名（Again / Hard / Good / Easy）はカードの上に重ねず、評価色の文字で 1 行分の領域に表示する。上・左・右のスワイプ中はカード直下の領域に出し、下（Again）のスワイプ中は下がるカードに隠れないようカード上側の領域に出す（そのとき直下の領域は空）。指を離したら元（上側は空、直下は表面なら案内文、裏面なら空）に戻す。横スワイプでは最大 8 度カードを傾ける。120px に達したら色を最大にして、指を離すとカードがその方向に抜けて次のカードが出る。左上の小さな評価表記は置かない |
| スワイプ方向の目印 | カードの四辺のすぐ外側に小さな目印を常時表示する。上「↑ Easy」、下「↓ Again」、左「← Hard」、右「→ Good」。それぞれの評価色を薄くした文字（不透明度 0.45、12px）で、表面・裏面の両方に出す。スワイプ中はその方向の目印だけを評価色の濃い文字（不透明度 1）にし、他は薄いままにする。左右の目印は縦書き（文字は正立）にして画面端の余白に置き、カードの幅をなるべく保つ。目印はカード要素の外に置いて追従させず、`pointer-events: none` でスワイプの判定にも関わらない。上の目印は上側の 1 行分の領域とカードの間に置き、領域と重ねない |
| 実機で動く条件 | カード要素に `touch-action: none` を付け、Pointer Events と `setPointerCapture` で追跡する。ドラッグ中は `touchmove` の既定動作を止め（`passive: false`）、学習画面に `overscroll-behavior: none` を付ける |
| 評価後 | 確認表示（トースト）は出さない。誤操作は ↶ の取り消し（6-3、7-6）で戻す |
...
| 要素 | 仕様 |
|---|---|
| 検索欄 | 画面上部（ヘッダーの下）。1 文字以上入力すると（入力から 300ms 後）、フォルダ一覧の代わりに全フォルダの単語から英語と日本語訳の部分一致（大文字小文字を無視）で検索した結果を表示する。各行: 英単語、日本語訳、フォルダ名。タップで単語フォーム（編集）へ。該当なしなら「該当する単語がありません」。結果が 100 件を超えるときは英単語順の先頭 100 件だけを表示し、絞り込みを促す。空にすると元の表示に戻る。サマリーカード、ストリーク、状態別割合は検索中もそのまま表示する |
| サマリーカード | 3 分岐。いずれも「見出し 1 行 + ボタン」の構造で、タップの対象はカード全体ではなくボタン。①復習あり: 見出し「今日の復習: N語 · 約M分」（M = ceil(N × 30 ÷ 60)）と、強調色（`--accent`）の大きなボタン「復習を始める」。ボタンで全フォルダ復習のモード選択へ。②復習 0 件で New あり: 見出し「今日の復習はありません。新しい単語が N語あります」と、①と同じ強調色・同じ大きさのボタン「新しい単語を学習する」。ボタンで全フォルダ New のみへ。①と②のカードは同じ配色・同じ大きさにし、応援メッセージに見えないようにする。③どちらも 0: 見出し「今日の復習はありません」のみでボタンなし。カード全体を薄い色にする |
| ストリーク | 6-7 の 3 パターン |
| 状態別割合 | 全単語の state 別件数と割合を 4 色の横棒で表示。下に凡例（New N / Learning N / Review N / Relearning N）。単語 0 件なら非表示 |
| 説明（ⓘ） | 右上、歯車の左に置く ⓘ のボタン。押すと下から「状態と評価の説明」のシート（7-9 の InfoSheet）を出し、閉じるボタンで閉じる。ⓘ と歯車のアイコンは 26px にし、タップ領域は 44px のままにする |
| フォルダ一覧 | 各フォルダをフォルダのアイコン（📁）を付けた角丸のカード（枠線と薄い影あり）で表示し、単語一覧の行（区切り線のリスト行。7-3）と同じ見た目にならないようにする。カード: フォルダ名、総単語数、今日の復習数。タップで単語一覧へ。カード右端の「…」でメニューを出す。「…」はカードの上下中央にそろえ、タップ領域は 44px 以上にする（文字は行の下端に寄るので、点を 3 つ描いて中央にそろえる）。メニューは他の確認ダイアログと同じ枠・背景・角丸のシート（`<dialog>`）で、見出しはフォルダ名。項目は「名前を変更」（名前変更ダイアログ）と「削除」（確認ダイアログ。配下の単語と履歴も削除）の 2 つで、それぞれ高さ 48px 以上の行にし、行の間に区切り線を入れ、押した瞬間に背景色が変わるようにする。「削除」は Again 色の文字。一番下に「キャンセル」を置く |
...
| フィルター | 「すべて」「要復習」「習得済み」。すべて = 全件。要復習 = state ≠ 0 かつ due ≤ 今日の終わり。習得済み = state = 2 かつ stability ≥ 30。絞り込みだけを行い、フィルター別の既定の並び順は持たない（並びは「並べ替え」に一本化する） |
| 並べ替え | フィルターの右に置くボタン。押すと下から次の 5 つを選ぶシートを出す。①登録日（新しい順） ②登録日（古い順） ③アルファベット順（A→Z） ④学習状態（New → Learning → Relearning → Review の順。同じ状態内は due 昇順） ⑤次回の復習日（近い順。未学習は最後）。既定は①。選択は保存せず、一覧を開くたびに既定に戻す。並べ替えはフィルターと検索の結果に対して適用する。ボタンには現在の並べ替え名を短く表示する（順に「新しい順」「古い順」「A→Z」「学習状態」「復習日」）。シートの各行は高さ 48px 以上、行の間に区切り線、押した瞬間に背景色が変わる。現在の選択にチェックを付け、一番下に「キャンセル」を置く |
| 件数表示 | フィルターか検索が効いているとき「N語中M語を表示中」 |
| 状態別割合 | このフォルダの単語で 7-2 と同じ横棒 |
| 一覧の行 | 区切り線で区切ったリスト行（背景付きの角丸カードにはしない。ホームのフォルダカードと見分けるため）。英単語、日本語訳、状態アイコン、復習予定。復習予定: state = 0 は「未学習」、due の日付が今日より前は「N日超過」、今日は「今日」、明日は「明日」、それ以外は「N日後」。行の並び順は「並べ替え」で選んだものに従う。タップで編集へ |
| 追加（＋） | 右上の「＋」を押すと、メニューを出さずに直接「単語を追加」の画面（単語フォーム。7-4）を開く。CSV/TSV 取込の入口は単語フォーム（新規）の中に置き、「＋」のメニューと「…」メニューは置かない |
| 選択（複数選択と一括移動・削除） | 右上の「選択」を押すと選択モードになり、各行の左にチェックボックスが出て、右上は「完了」に変わる。下部の開始ボタンは横に並べた 2 つのボタン「移動」と「N件を削除」（N は選択数。どちらも 0 件なら非活性）に変わる。「移動」を押すと移動先フォルダの一覧をダイアログ（見出し「N件の移動先」）で出し、選ぶと選択中の単語をまとめて移動する。一覧と「移動先が無い」の判定の両方から現在のフォルダを除き、現在のフォルダ以外が 0 件なら一覧の代わりに「移動先のフォルダがありません」を出す。ダイアログは他の確認ダイアログと同じ見た目（`<dialog>`、枠線なし）で、各行は枠線のないボタン。フォーカス枠はキーボード操作（`:focus-visible`）のときだけ出し、タップや、開いたときの自動フォーカスでは出さない（開いたときのフォーカスは最初の行ではなくダイアログ自身に置く。他のダイアログも同じ）。移動は folderId と updatedAt の更新のみで、FSRS の状態と ReviewLog は変えず、1 トランザクションで行う。移動後は選択モードを抜け、画面下（スワイプ削除の「元に戻す」と同じ位置のトースト）に「N件を「移動先のフォルダ名」に移動しました」を 4 秒表示する。「N件を削除」を押すと確認ダイアログ「N件の単語と学習履歴を削除します。よろしいですか？」の後に単語と ReviewLog を 1 トランザクションで削除する。「完了」で選択を解除してモードを抜ける。選択モード中はスワイプ削除を使わない。単語 0 件なら「選択」は非活性 |
| 左スワイプで削除 | 行を左にスワイプすると、右端からゴミ箱アイコンが移動量に応じて徐々に現れる。行幅の 20% 以上スワイプした状態で指を離すと削除、20% 未満なら元の位置に戻す。確認ダイアログは出さず、行を一覧から隠して画面下に「削除しました　元に戻す」を 5 秒表示し、「元に戻す」を押すと行を戻す。実際の削除（単語と ReviewLog）はこの表示が消えた時点、または画面遷移（単語一覧のアンマウント）の時点で行う。横方向の移動が縦方向より大きい場合だけスワイプとして扱い、縦方向はブラウザのスクロールに任せる（`touch-action: pan-y`）。保留中に別の行をスワイプしたら、先の保留分を先に削除する |
...
| フォルダ | 英単語欄の上に置く選択欄（新規・編集の両方）。全フォルダから選ぶ。初期値は現在のフォルダ（新規は開いた一覧のフォルダ、編集はその単語の所属フォルダ）。保存時に folderId を更新する（FSRS の状態と ReviewLog は変えない）。変更があればキャンセル時の破棄確認の対象になる |
| 英単語 | 必須。`lang="en"`、autoCapitalize/autoCorrect/spellCheck を無効。200 文字まで |
| 日本語訳 | 必須。プレースホルダー「例: 曖昧な,あいまいな（カンマ区切りで複数可）」。500 文字まで |
| メモ | 任意。複数行。1000 文字まで |
| 取込への導線（新規のみ） | メモ欄の下に薄い文字のリンク「複数の単語をまとめて登録する（CSV / TSV 取込）」を置く。押すとファイル選択の後に取込画面（10-3）へ遷移する。取り込む先はフォルダ欄で選んでいるフォルダで、取込後はその一覧へ戻る。取込画面のキャンセルはこのフォームに戻る。編集（既存の単語）では表示しない |
| 保存 | 必須項目が trim 後に空、または文字数超過なら非活性。超過項目の下に赤字で理由を表示。新規は `newCardFields()` を付けて保存。保存後はフォルダ欄で選んだフォルダの単語一覧へ戻る（フォルダを変えた場合は移動先の一覧）。編集でフォルダを変えた場合は、戻った一覧の画面下に「1件を「移動先のフォルダ名」に移動しました」を 4 秒表示する（7-3 の一括移動と同じ文面・位置） |
| キャンセル | 変更があれば「変更を破棄しますか？」。元のフォルダの単語一覧へ戻る |
...
| 本体 | 6-4 |
| 取り消し | 左上の ↶ ボタン。押すと直前の評価を取り消す（6-3 の `undo()`）。セッション開始時点まで何回でも戻れる。取り消せるものがないときは非活性。確認ダイアログは出さない |
...
| 内訳 | 上から Again、Hard、Good、Easy の順に、件数を 4 色の横棒と数字で表示。色は 5-3 のとおり |
| 正答率、所要時間、次回最も早い復習日 | 6-6 |
| ストリーク | 当日初の学習なら「🔥 N日連続学習！」 |
| 学習を終える | 通知が ON のとき「今日の学習を終える」ボタン（第9章）。ホームと同じ処理 |
| ボタンの配置 | 「ホームへ」と「もう一度」は縦並びにして画面の下部に固定し、それぞれ横幅いっぱい・高さ 48px 以上にする。下端はセーフエリア分の余白を取る。一番下に「ホームへ」（強調色 `--accent`）、その上に「もう一度」（枠線のみ）を置く。内訳などの内容はその上でスクロールできる |
| ホームへ | ホームへ戻る |
...
| データ | 「バックアップを書き出す」「バックアップを読み込む」「進捗をリセット」の 3 つを、枠線・角丸・高さ 48px 以上のボタンにする。書き出しと読み込みは強調色（`--accent`）の枠と文字、進捗リセットは Again 色の枠と文字で、いずれも背景は塗らず、押した瞬間に背景色が変わるようにする。CSV/TSV 取込の説明とバックアップの注意はボタンの下に置き、ボタンと区別できるよう薄い文字（`--muted`）のままにする。進捗リセットはフォルダ選択または「すべて」→ 確認 → 実行 |
| アプリ情報 | 「状態と評価の説明」の行（押すとホームの ⓘ と同じシート、7-9 の InfoSheet を開く）。バージョン（package.json の version）。ライセンス表記: ts-fsrs（MIT）、Dexie（Apache-2.0）、vite-plugin-pwa（MIT）。更新があれば「更新して再読み込み」ボタン |

### 7-9. 共通コンポーネント

| 名前 | 用途 |
|---|---|
| SummaryCard | ホームの 3 分岐カード。見出し 1 行と、ボタン（強調色。どちらも 0 のときはなし）で構成する（7-2） |
| StateBar | 状態別割合の横棒と凡例 |
| ProgressBar | 学習画面の進捗 |
| ConfirmDialog | 削除、終了、リセット、破棄の確認。`<dialog>` 要素を使う |
| EmptyState | 空状態の文言 |
| UpdateBanner | Service Worker の更新通知 |
| InfoSheet | 状態と評価の説明。下から出すシート（`<dialog>`。背景・角丸は確認ダイアログと同じで、上端だけ丸める）。ホームの ⓘ（7-2）と設定のアプリ情報の行（7-8）の両方から開き、閉じるボタンで閉じる。文面は下記のとおりにそのまま表示する |

InfoSheet の文面（見出しは「状態」「評価」「補足」）。改行もこのとおりにする:
...
状態のアイコンと名前は 5-3 の表と同じものを使い、評価の名前は 5-3 の色（Again / Hard / Good / Easy 色）で表示する。
...
| 画面端の引っ張り | `html` と `body` に `overscroll-behavior: none` を付け、学習画面以外でも画面全体が端で引っ張られないようにする。各画面の縦スクロールはそのまま動く |
| 高さ | `100dvh` を使う |
...
### 10-3. CSV / TSV 取込（`src/app/csv.ts`、`src/ui/screens/Import.tsx`）

単語フォーム（新規）の「複数の単語をまとめて登録する（CSV / TSV 取込）」（7-4）でファイルを選ぶと、単語フォームの代わりに取込画面を表示する（URL はフォームのまま）。取り込む先はフォームのフォルダ欄で選んでいるフォルダ。「取り込む」を押すまで何も書き込まない。
...
| プレビュー | 画面上部に先頭 5 行の表。「1行目は見出し」が ON なら 1 行目を見出しの体裁で表示する。表の上にファイル名と取り込む行数 |
| 区切り文字 | セレクトでカンマ / タブ / セミコロンから選ぶ。初期値: 前回使った区切り（settings の `importDelimiter`）が 1 行目に含まれていればそれ、含まれていなければ自動判定（1 行目にタブがあればタブ、なければセミコロンがありカンマがなければセミコロン、それ以外はカンマ） |
| 1行目は見出し | トグル。初期値: 1 行目のいずれかのセルが下記の見出しの語で始まれば ON、そうでなければ前回の値（`importHasHeader`） |
| 列の割り当て | プレビュー表の各列の上に「英単語 / 日本語訳 / メモ / 使わない」のセレクト。英単語・日本語訳・メモは 1 列ずつ。ほかの列に付いている役割を選ぶと、その列は「使わない」に戻る。初期値の優先順: ①「1行目は見出し」が ON で、見出しの語から英単語と日本語訳の両方が決まればそれ（同じ役割が複数あれば左の列だけ）。②前回の割り当て（`importColumns`）を列数に合わせて切り詰め / 「使わない」で埋めたものに英単語と日本語訳が含まれていればそれ。③左から 英単語, 日本語訳, メモ |
| 見出しの語 | 英単語 = 「英単語」「英語」「english」「word」「term」、日本語訳 = 「日本語」「訳」「japanese」「meaning」「definition」「translation」、メモ = 「メモ」「備考」「memo」「note」「comment」のいずれかで始まるセル。大文字小文字は区別しない |
| 区切り・見出しの変更 | 変更すると列の割り当てを上記の初期値の規則で作り直す |
| 取り込むボタン | 英単語と日本語訳が 1 列ずつ選ばれ、取り込む行（見出しを除く）が 1 行以上あるまで非活性。非活性の間は「英単語と日本語訳の列を1つずつ選んでください」を表示 |
| 検証 | 4-3 のバリデーション。違反行はスキップし件数を報告。空行は無視 |
| 重複 | 同じフォルダに大文字小文字を無視して同じ英単語があればスキップ。ファイル内の重複も 2 件目以降をスキップ |
| 保存 | 「取り込む」を押すと、まず区切り・見出しの有無・列の割り当てを settings（4-5）に保存して次回の初期値にする。続けて `newCardFields()` を付けた単語を 1 トランザクションでまとめて追加し、フォルダ欄で選んだフォルダの単語一覧に戻って結果を表示する。バッジを更新する |
| 結果 | 「N件を取り込みました（スキップ M件: 重複 a件、形式エラー b件）」 |
| キャンセル | 何も書き込まず単語フォームに戻る（入力途中の内容は残る）。settings も変えない |

日本語訳にカンマ区切りの複数候補を書く場合は、CSV ではダブルクォートで囲む。TSV やセミコロン区切りならそのまま書ける。設定画面の説明にこの旨と取込の入口を書く。
...
| `domain/fsrs.ts` | New に Good で state = 1（学習ステップ 2 段目）と due が約 10 分後、その Learning に Good で state = 2 と due が 1 日以上先、Review に Again で state = 3 と lapses + 1、New に Easy で state = 2、preview が 4 件のラベルと requeue を返す（state 1 か 3 なら「↻」、それ以外は「N日後」）、toCard/fromCard の往復 |
| `domain/dates.ts` | 日付キー、今日の終わり、相対表示（超過 / 今日 / 明日 / N日後） |
| `app/queue.ts` | 復習優先、New へのフォールバック、上限の切り詰め、シャッフル時に要素集合が同じ |
| `app/session.ts` | 初回評価のみ集計、Learning への遷移で末尾に再出題、完了判定、途中終了、正答率、取り消しで状態・履歴・再出題・集計が評価前に戻る |
...
| `db/repo.ts` | fake-indexeddb 上で、フォルダ削除のカスケード、復習対象クエリ、進捗リセット、評価の取り消し（FSRS 項目の書き戻しと ReviewLog の削除） |
...
| 4 | スワイプ評価 | 表面・裏面のどちらでも 4 方向が正しい評価になり、120px 未満では元に戻る。表面のタップは裏返る。スワイプ中に画面がスクロールしたりラバーバンドしたりしない |
| 4-2 | 評価の取り消し | ↶ で直前の評価が取り消され、その単語が再び出る（フラッシュカードは裏面、タイプ入力は入力欄が空）。単語一覧の復習予定が評価前に戻る |
